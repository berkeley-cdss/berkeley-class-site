...
---
### We can set the release date and time for lectures so that the links on the home page won't appear until after the release times.
For example if we set the front matter of the lecture md file to be:
```yaml
date: 2025-9-28
time: 15:00
```
Then this link won't appear on the homepage until September 28, 2025 at 3pm. By default, if we don't define the release time, then lectures will be released at 10am. This is defined in `_config.yml` under `defaults` for lectures.
