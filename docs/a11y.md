...
permalink: /docs/web-accessibility/
published: true
layout: page
...
As of June 2024, all Berkeley websites must meet the WCAG 2.0 AA standards. In April 2026, all Berkeley websites must meet WCAG 2.1 AA. Visit Berkeley's [Digital Accessibility page](https://dap.berkeley.edu/) for the most up to date information. 

WCAG 2.2 AA is the most recent version of the standard, so the tests include optional checks for both standards.
...
After passing all tests, add the following bagde to your README
(Just make sure to replace `ORG/REPO` with your GitHub repo path.) [More on workflow status badges](https://docs.github.com/en/actions/how-tos/monitor-workflows/add-a-status-badge)
